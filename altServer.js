...
//notion-cs-content.onrender.com/

// Middleware to fetch and cache the database on any GET request
https: app.use(async (req, res, next) => {
  const allowedOrigins = [
    "http://localhost:5173",
    "http://localhost:5174",
    "https://cslearnings.vercel.app",
  ];
  ...
  if (req.method === "GET" && !cachedDatabase) {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch initial database" });
    }
  }
  next();
});
...
function filtersApplied(thePages) {
  return thePages.filter(
    (page) => page.Source === "Career" && page.Tags.length === 3
  );
}
...
app.get("/cstools", (req, res) => {
  ...
  try {
    const cstools = cachedDatabase.filter((page) =>
      page.Tags.some(
        (str) => typeof str === "string" && str.includes("CS Tools")
      )
    );
...
app.get("/reactjs", (req, res) => {
  ...
  try {
    const reactjspages = cachedDatabase.filter((page) =>
      page.Tags.some(
        (str) => typeof str === "string" && str.includes("ReactJS")
      )
    );
    // .includes("(LinkedIn) Mike Peditto"));
...
app.get("/potentialresources", (req, res) => {
  ...
  try {
    const potentialResources = cachedDatabase.filter((page) =>
      page.Tags.some(
        (str) => typeof str === "string" && str.includes("Potential Resource")
      )
    );
    // .includes("(LinkedIn) Mike Peditto"));
  ...
  try {
    ...
    if (page && page.id) {
      ...
      if (page) {
        res
          .status(404)
          .json({ error: `Page found but content could not be retrieved` });
      } else {
        res
          .status(404)
          .json({ error: `Neither Page nor content could be retrieved` });
      }
    }
  } catch (error) {
